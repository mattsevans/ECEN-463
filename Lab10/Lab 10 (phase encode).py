...
secondary = 1
num_avg = 64
f_start = 3.366*10**6 #reset 3.33
f_stop = 3.366*10**6 #reset 3.37
step = 1000
...
num_points = 64
scale_factor = 1
factor = 0.1    # this is relating gradient pulse to dephasing
...
phase_list = np.linspace(0, 0, int(phase_num), endpoint = False)
...
# trapezoid 
factor_list = np.linspace(1, -1, phase_num, endpoint = True)
...
for factor in factor_list:      
    for theta in theta_array:
        for f in f_range:
            ...
            np.savetxt("proj" + str_theta + " phase(complete magnitude).csv",magnitude,delimiter=",",fmt = "%.6e")
            ...
            f_band = f_pos[band_mask]
            mag_band = mag_pos[band_mask]
            
            mid_idx = len(f_band) // 2
            half_pts = NFE //2  
            left_idx = max(0,mid_idx - half_pts)
            right_idx = min(len(f_band) - 1,mid_idx+half_pts)
            
            f_mid = f_band[mid_idx]
            mag_mid = mag_band[mid_idx]
            
            
            plt.plot(f_band / 1000, mag_band, label='FFT (90–110 kHz)')

            # Mark middle, left, and right points
            # plt.scatter(f_band[mid_idx] / 1000, mag_band[mid_idx])
            # plt.scatter(f_band[left_idx] / 1000, mag_band[left_idx])
            # plt.scatter(f_band[right_idx] / 1000, mag_band[right_idx])
            ...
            IF_plt = np.where(f_pos == 100000)[0][0]
            NFE_pts = X[IF_plt - NFE//2:IF_plt + NFE//2]
            ...
            win_len = 64
            if len(f_band) < win_len:
                win_len = len(f_band)   # just in case
            
            mid_idx = len(f_band) // 2
            half = win_len // 2
            
            start_idx = max(0, mid_idx - half)
            end_idx   = start_idx + win_len   # end is exclusive
            
            
            if end_idx > len(f_band):
                end_idx = len(f_band)
                start_idx = end_idx - win_len
            
            
            f_win  = f_band[start_idx:end_idx]
            mag_win = mag_band[start_idx:end_idx]
            
            
            # plt.plot(f_win / 1000, mag_win, marker='o')  # only 64 points
            ...
            plt.plot(f_pos[IF_plt - NFE//2:IF_plt + NFE//2] / 1000, mag_pos[IF_plt - NFE//2:IF_plt + NFE//2])
            ...
            plt.title('FFT of signal @ ' + str(theta) + ' factor: (' + str(factor) + ',' +str(offset1) + ')')
            plt.xlabel('Frequency (kHz)')
            plt.ylabel('Magnitude')
            #plt.xlim(80,120)
            
            
            np.savetxt("proj" + str_theta + " phase "+ str(round(factor, 3))  + ".csv",X,delimiter=",",fmt = "%.6e")
            print('CSV file created succesfully')
            
            NFE_pts.tofile("proj" + str_theta + " (64 points) phase " + str(factor) + ".csv", sep = ',')
...
np.savetxt("img_data_2.csv", k_space, delimiter=",", fmt = "%.6e")
plt.imshow(abs(fft2(k_space)))
# num = max(snr_list)
# print(num)
